...
case 'ExportClustersOne'
	...
	MClust_Clusters{clusterStart} = precut(['KKwik Cluster']);
	MClust_Colors(1+clusterStart,:) = color;
	for iC = 1:nC
		if keepYN(iC)
			MClust_Clusters{clusterStart} = AddIndices(MClust_Clusters{clusterStart}, ...
				FindInCluster(KlustaKwik_Clusters{iC}));
		end%if
	end%for
	...
	exportType = get(exportTypeHandle, 'String');
	exportType = exportType{get(exportTypeHandle, 'Value')};
	MClust_Clusters{clusterStart} = feval(exportType(2:end), GetName(MClust_Clusters{clusterStart}), MClust_Clusters{clusterStart});
